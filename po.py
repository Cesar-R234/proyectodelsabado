...
laser_sonido = pygame.mixer.Sound("sonidos/laser.mp3")
explosion_sonido = pygame.mixer.Sound("sonidos/explosion.mp3")
golpe_sonido = pygame.mixer.Sound("sonidos/golpe.mp3")
...
def texto_puntacion(frame , text , size, x,y):
...
class jugardor(pygame.sprite.Sprite):
    def __init__(self):
        super().__init__()
...
class Enemigos(pygame.sprite.Sprite):
    ...
    def disparar_enemigos(self):
        ...
        grupo_jugador.add(bala)
        grupos_balas_enemigos.add(bala)
        laser_sonido.play()
    
class Balas(pygame.sprite.Sprite):
    def __init__(self, x, y ):
        super().__init__()
        self.image = pygame.image.load("images/bala.png").convert_alpha()
        self.rect = self.image.gte_rect()
        self.rect.centerx = x
        self.rect.y = y 
        self.velocidad = -18
    ...
    def update(self):
        ...
        if self.rect.botton <0 :
            self.kill()
            
class Balas_enemigos(pygame.sprite.Sprite):
    def __init__(self, x, y):
        super().__init__()
        self.image = pygame.image.load("images/icono.png").convert_alpha()
...
grupos_balas_enemigos = pygame.sprite.Group()
...
for x in range(10):
    ...
    colicion2 = pygame.sprite.spritecollide(player , grupos_balas_enemigos, True)
    for j in colicion2:
        player.vida -= 10
        if player.vida <=0:
            run = False
    ...
    texto_puntacion(window, ("Score: "+ str(score)+"     "),30, width -85, 2  )
